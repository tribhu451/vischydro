...
#ifndef _TH2_sVECTOR_3D_H_
  #define _TH2_sVECTOR_3D_H_
...
// CLASS FUNCTIONS
    ...
    int         GetZPts() const;
    void        put(double x, int i  , int j , int k);

    double	Interpolate(double aX, double aY, double aZ);
    sVector3D*	DerivativeX(const char* aName);
    sVector3D*	DerivativeY(const char* aName);
    sVector3D*	DerivativeZ(const char* aName);
    

    // added by Piotr Bozek derivates with flux control for hydro
  
    double derx(int ix, int iy, int ieta);
    double dery(int ix, int iy, int ieta);
    double dere(int ix, int iy, int ieta);
