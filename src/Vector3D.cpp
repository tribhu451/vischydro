...
std::string	Vector3D::GetName() const	{ return mVecName; }
...
void Vector3D::put(double x,int i,int j,int k)
{
  mVec[i][j][k]=x;
}
...
double Vector3D::Valx(int i)
{
  return mXmin + i * (mXmax - mXmin) / (mXpts - 1);
}

double Vector3D::Valy(int j)
{
  return mYmin + j* (mYmax - mYmin) / (mYpts - 1);
}

double Vector3D::Valz(int k)
{
  return mZmin + k *(mZmax - mZmin) / (mZpts - 1);
}


double Vector3D::derx(int i, int j, int k){
...
double Vector3D::dery(int i, int j, int k){
  //     if (i==0||j==0||k==0||i==(mXpts-1)||j==(mYpts-1)||k==(mZpts-1))
  //        {return 0.;}
  if (j==0){
...
double Vector3D::dere(int i, int j, int k){
  //      if (i==0||j==0||k==0||i==(mXpts-1)||j==(mYpts-1)||k==(mZpts-1))
  //      {return 0.;}
  if (k==0){
...
double Vector3D::minmod2(double x, double y){
  return y;
  //     return 0;
  if ((x>0)&&(y>0)){
    return    (x < y) ? x : y;
...
double Vector3D::minmod3(double x, double y, double z){
  //   return 0;
  return z;
  if ((x>0)&&(y>0)&&(z>0)){
        x=(x < y) ? x : y;
